...
echo "Admin role arn is : ${AdminIamRoleArn}"
echo "Region is : ${region}"

export KUBECONFIG=/tmp/kubeconfig

update_kubeconfig(){
    echo "Retrieving kubeconfig details and saving to "
    aws eks update-kubeconfig --name "${ClusterName}" --role "${AdminIamRoleArn}" --kubeconfig ${KUBECONFIG}
}
...
KF_STAGING_CONFIG_URL="s3://${KubeflowStagingS3Bucket}/${ClusterName}/config.xml"
